    ...
    render(){
        ...
        if (this.props.focusarea){
            ...
            var header = <Panel className="focus-area-header">
                    ...
                    <div className="text-center" key={uuid.v4()}><strong><h2 className="text-center">{this.props.focusarea.focusAreaInfos.courses[0].name}</h2></strong></div>
                    <br />
            ...
            var resultComponents = this.props.focusarea.playlists.map(function(result) {
                    ...
                    var objComponents = result.objectives.map(function(objresults) {
                        return <li className="format-table-content" key={uuid.v4()}> {objresults} </li>
                    })
                    
                    return <Panel className="focus-area-body results-content" key={uuid.v4()} onClick={(e) => component.handleClick(e, result.title)}>
            
                        <div className="format-table-content"  key={uuid.v4()}><h4> {result.title}</h4></div>
